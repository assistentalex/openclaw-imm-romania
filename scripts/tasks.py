...
def cmd_list(args: argparse.Namespace) -> None:
    ...
    if args.overdue:
        now = datetime.now()
        items = [t for t in items if t.due_date and t.due_date < now and str(t.status) != "Completed"]
...
def cmd_get(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        out({
            "ok": True,
            "task": task_to_dict(task, detailed=True)
        })
...
def cmd_update(args: argparse.Namespace) -> None:
    ...
    if not updated:
        die("No fields to update. Use --subject, --body, --due, --start, --priority, --status, or --percent")
...
def task_to_dict(task: Task, detailed: bool = False) -> Dict[str, Any]:
    ...
    if detailed:
        result.update({
            ...
            "owner": task.owner,
            "delegation_state": str(task.delegation_state) if task.delegation_state else None,
...
def add_parser(subparsers: argparse.ArgumentParser) -> None:
    ...
    p_list.add_argument("--limit", "-n", type=int, default=20, help="Maximum number of tasks to return")
    p_list.add_argument("--status", choices=["not_started", "in_progress", "completed", "waiting", "deferred"],
                        help="Filter by status")
    p_list.add_argument("--completed", action="store_true", dest="completed_only", help="Show only completed tasks")
    p_list.add_argument("--all", "-a", action="store_true", dest="include_completed", help="Include completed tasks")
    ...
    p_create.add_argument("--priority", "-p", choices=["low", "normal", "high"], default="normal", help="Task priority")
    ...
    p_update.add_argument("--priority", "-p", choices=["low", "normal", "high"], help="New priority")
    p_update.add_argument("--status", choices=["not_started", "in_progress", "completed", "waiting", "deferred"],
                         help="New status")
    ...
    p_delete.add_argument("--hard", action="store_true", help="Permanently delete (no trash)")
