...
def cmd_reminders(args: argparse.Namespace) -> None:
    ...
    if overdue_tasks:
        ...
        for task in overdue_tasks:
            body_parts.append(f"<li><b>{task['subject']}</b> - {task['days_overdue']} days overdue (due: {task['due_date']})</li>")
        body_parts.append("</ul>")
    ...
    if upcoming_tasks:
        ...
        for task in upcoming_tasks:
            body_parts.append(f"<li><b>{task['subject']}</b> - due in {task['hours_until']} hours ({task['due_date']})</li>")
        body_parts.append("</ul>")
    ...
    if args.dry_run:
        _logger.info("Dry run - would send email", {
            "to": to_addr,
    ...
    try:
        ...
        _logger.info("Reminder email sent", {
            "to": to_addr,
            "overdue": len(overdue_tasks),
            "upcoming": len(upcoming_tasks),
        })
...
def cmd_status(args: argparse.Namespace) -> None:
    ...
    # Count overdue
    now = datetime.now()
    overdue = sum(1 for t in all_tasks if t.due_date and 
                   datetime(t.due_date.year, t.due_date.month, t.due_date.day) < now 
...
def add_parser(subparsers: argparse.ArgumentParser) -> None:
    ...
    p_sync.add_argument("--limit", "-n", type=int, default=50, help="Maximum tasks to return")
    p_sync.set_defaults(func=cmd_sync)
    
    # reminders
    p_reminders = subparsers.add_parser("reminders", help="Send email reminders for tasks")
    p_reminders.add_argument("--to", "-t", help="Recipient email (default: self)")
    p_reminders.add_argument("--hours", type=int, default=24, help="Hours ahead for upcoming tasks")
    p_reminders.add_argument("--dry-run", action="store_true", help="Show what would be sent")
    p_reminders.set_defaults(func=cmd_reminders)
    
    # link-calendar
    p_link = subparsers.add_parser("link-calendar", help="Create calendar event from task")
    p_link.add_argument("--id", "-i", required=True, help="Task ID")
    p_link.add_argument("--time", "-t", help="Start time (HH:MM, default 09:00)")
    p_link.add_argument("--duration", "-d", type=int, default=60, help="Duration in minutes")
    p_link.add_argument("--reminder", "-r", type=int, default=30, help="Reminder minutes before")
