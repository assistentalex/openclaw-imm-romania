...
class Config:
    ...
    def _validate_and_prompt(self) -> None:
        ...
        if not self._config.get("username"):
            if sys.stdin.isatty():
                ...
                die("Missing required configuration: username. Set EXCHANGE_USERNAME or use --username")
        ...
        if not self._config.get("email"):
            ...
            if "@" in username:
                ...
                die("Missing required configuration: email. Set EXCHANGE_EMAIL or use --email")
        ...
        if not self._config.get("password"):
            if sys.stdin.isatty():
                ...
                die("Missing required configuration: password. Set EXCHANGE_PASSWORD environment variable")
        ...
        if not self._config.get("autodiscover", True) and not self._config.get("server"):
            die("Server URL is required when autodiscover is disabled. Set EXCHANGE_SERVER or use --server")
    ...
    def save_to_file(self, path: Optional[str] = None) -> None:
        ...
        try:
            path_obj = Path(path)
            content = yaml.dump(config_to_save, default_flow_style=False) if HAS_YAML else json.dumps(config_to_save, indent=2)
